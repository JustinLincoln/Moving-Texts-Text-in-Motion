...
void setup() {
  ...
  movie = new Movie(this, "yourvideohere.mov");
  movie.loop();
...
// CHANGE BY frameCount
  if (frameCount % speedControl == 0) {     // every n'th frame
    ...
    //fill(BG_Gray, BG_AlphaControl);  // fills screen-sized rectangle (below) with white w/ opacity determined by midi
...
// QUICK RED
    ...
    text(words[wordIndex], xPos, yPos);
